...
export class SchemaRegistry {
	...
	loadFolder(folder: string): { loaded: number; errors: string[] } {
		...
		for (const file of fs.readdirSync(folder)) {
			...
			if (file === 'allEffects.json' || file === 'allSounds.json' || file === 'allAttributes.json' || file === 'allTeams.json') continue; // flat vanilla name lists, not class schemas - see NameListIndex
